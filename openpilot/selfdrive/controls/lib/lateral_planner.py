...
LATERAL_ACCEL_COST = Params().get("LatMpcAccelCost", return_default=True) * 0.01 if Params().get("LatMpcAccelCost", return_default=True) is not None else 0.0
LATERAL_JERK_COST = Params().get("LatMpcJerkCost", return_default=True) * 0.01 if Params().get("LatMpcJerkCost", return_default=True) is not None else 0.04
...
STEERING_RATE_COST = Params().get("LatMpcSteeringRateCost", return_default=True) if Params().get("LatMpcSteeringRateCost", return_default=True) is not None else 700
...
class LateralPlanner:
  def __init__(self, CP, debug=False):
    ...
    self.useLaneLineSpeedApply = self.params.get("UseLaneLineSpeed")
    self.pathOffset = self.params.get("PathOffset")
    ...
    self.lateralPathCost = self.params.get("LatMpcPathCost") * 0.01
    self.lateralMotionCost = self.params.get("LatMpcMotionCost") * 0.01
  ...
  def update(self, sm, carrot):
    ...
    if self.readParams <= 0:
      ...
      self.lateralPathCost = self.params.get("LatMpcPathCost") * 0.01
      self.lateralMotionCost = self.params.get("LatMpcMotionCost") * 0.01
      LATERAL_ACCEL_COST = self.params.get("LatMpcAccelCost") * 0.01
      LATERAL_JERK_COST = self.params.get("LatMpcJerkCost") * 0.01
      STEERING_RATE_COST = self.params.get("LatMpcSteeringRateCost")
