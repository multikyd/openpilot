...
class DesireHelper:
  ...
  def _update_params_periodic(self):
    if self.frame % 100 == 0:
      ...
      self.modelTurnSpeedFactor = self.params.get("ModelTurnSpeedFactor")
  ...
  def update(self, carstate, modeldata, lateral_active, lane_change_prob, carrotMan, radarState):
    ...
    if desire_enabled:
      ...
      side_object_dist = radar.dRel + radar.vLead * 4.0 if radar.present else 255
    ...
    # 1) turn 후 일정시간 동안은 아무 것도 하지 않음
      ...
      # ─ Lane Change FSM 처리 ─
        ...
        # LaneChangeState.laneChangeStarting
          ...
          if 0.1 > self.lane_change_ll_prob >= 0.01:
