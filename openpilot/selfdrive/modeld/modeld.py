...
def main(demo=False):
  ...
  sm = SubMaster(["deviceState", "carState", "narrowRoadCameraState", "liveCalibration", "driverMonitoringState", "carControl", "liveDelay", "carrotMan", "radarState"])
  ...
  lat_smooth_seconds_dynamic = 0.0
  ...
  while True:
    ...
    if custom_lat_delay > 0.0:
      lat_delay = custom_lat_delay + lat_smooth_seconds_dynamic
    else:
    ...
    if model_output is not None:
      ...
      action = get_action_from_model(model_output, prev_action, lat_delay + DT_MDL, long_delay + DT_MDL, v_ego, lat_smooth_seconds_dynamic, vEgoStopping)
      prev_action = action
