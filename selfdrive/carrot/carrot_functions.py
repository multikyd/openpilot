...
class CarrotPlanner:
  ...
  def dynamic_t_follow(self, t_follow, lead, desired_follow_distance, a_prev):
    ...
    if self.desireState > 0.9 and self.desireStateCount < int(1.5 / DT_MDL):  # lane change state, 1.5초동안만.
      ...
      t_follow += np.interp(a_prev[0], [-2.0, -0.5], [0.1, 0.0])
