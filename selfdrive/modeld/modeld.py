...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
                          long_action_t: float, v_ego: float, lat_smooth_seconds: float, vEgoStopping: float) -> log.ModelDataV2.Action:
    ...
    desired_curvature = desired_curv_unscaled / 100
...
def main(demo=False):
  ...
  while True:
    ...
    if model_output is not None:
      ...
      action = get_action_from_model(model_output, prev_action, long_action_t, v_ego, lat_smooth_seconds_dynamic, vEgoStopping)
      prev_action = action
